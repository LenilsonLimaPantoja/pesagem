                        ...
                        <div className={styles.area_card_caixa}>
                            {caixas?.map((item) => (
                                <div key={item?.id} className={styles.card_caixa} title={`Visualizar gráfico da caixa ${item?.id}`}>
                                    <div className={styles.card_click} onClick={item?.peso_atual ? () => navigation(`/caixa/relatorio/${item?.id}`) : () => AlertErro(`Peso não registrado para a caixa ${item?.identificador_balanca}`)} />
                                    ...
                                    <div className={styles.btns_card}>
                                        ...
                                        <button onClick={() => handleRemoverCaixaConfirme(item?.id)}>CALA A BOCA</button>
                                    </div>
                                </div>
                            ))}
                        </div>
